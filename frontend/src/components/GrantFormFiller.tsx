...
export const GrantFormFiller: React.FC<GrantFormFillerProps> = ({ grantId }) => {
  ...
  const fillGrantForm = async () => {
    ...
    try {
      ...
      // Extract chat history for multi-agent framework - NO FALLBACKS
      if (processingMode === 'multi-agent-framework') {
        if (data.chat_history && data.chat_history.length > 0) {
          setChatHistory(data.chat_history);
          setShowChatWindow(true);
        } else {
          // NO FALLBACKS - Surface the error
          throw new Error(`Multi-agent framework failed to return chat_history. Response: ${JSON.stringify(data)}`);
        }
      }
