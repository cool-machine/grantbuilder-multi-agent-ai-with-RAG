...
import { GrantAnalysisService } from './grantAnalysis';
...
export class DocumentExtractor {
  ...
  async extractText(file: File, onProgress?: (status: ProcessingStatus) => void): Promise<ExtractedDocument> {
    ...
    try {
      ...
      // Try tokenization (skip AI analysis until Azure OpenAI is configured)
      onProgress?.({
        id: file.name,
        status: 'extracting',
        progress: 96,
      ...
      try {
        ...
        tokenizedData = await tokenizer.tokenizeText(
          ...
          (progress, message) => {
            onProgress?.({
              id: file.name,
              status: 'extracting',
              progress: 96 + (progress * 0.03),
              message: `Tokenization: ${message}`
            });
          }
        );
        
        // TODO: Enable AI analysis once Azure OpenAI is configured
        // const grantAnalyzer = GrantAnalysisService.getInstance();
        // const analysisResult = await grantAnalyzer.processDocument(extractedText, file.name, file.type, onProgress);
        ...
        onProgress?.({
          ...
          status: 'extracting',
          progress: 99,
          message: `Tokenization failed: ${tokenError instanceof Error ? tokenError.message : 'Unknown error'}. Continuing without tokens...`
        });
      }
