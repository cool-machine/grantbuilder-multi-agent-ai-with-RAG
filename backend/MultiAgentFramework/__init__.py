...
class MultiAgentOrchestrator:
    ...
    def _create_field_responses(self) -> Dict[str, str]:
        """
        Create individual field responses from completed tasks
        """
        # Map tasks to grant form fields
        field_mapping = {
            "organization_name": "TechStart AI - Leading AI Research Organization",
            "mission_statement": "To advance artificial intelligence research for social good through innovative solutions that address global challenges while ensuring ethical AI development and deployment.",
            "project_title": "AI-Powered Grant Discovery and Application Optimization Platform",
            "project_duration": "24 months (2-year comprehensive development and deployment phase)",
            "requested_amount": "500000",
            "contact_information": "Dr. Jane Smith, Principal Investigator | Email: j.smith@techstartai.org | Phone: (555) 123-4567 | Address: 123 Innovation Drive, Tech Valley, CA 94000"
        }
        
        # Create responses based on completed task results
        responses = {}
        for field, default_response in field_mapping.items():
            # Try to find relevant task result
            relevant_task = None
            for task in self.tasks:
                if task.status == TaskStatus.COMPLETED.value:
                    if "research" in task.description.lower() and field in ["organization_name", "mission_statement"]:
                        relevant_task = task
                        break
                    elif "budget" in task.description.lower() and field == "requested_amount":
                        relevant_task = task
                        break
                    elif "writing" in task.description.lower() and field in ["project_title", "project_duration"]:
                        relevant_task = task
                        break
                    elif "networking" in task.description.lower() and field == "contact_information":
                        relevant_task = task
                        break
            
            if relevant_task and relevant_task.result:
                # Use task result with field-specific enhancement
                responses[field] = f"{relevant_task.result} - Generated by {self.agents[AgentRole(relevant_task.assigned_to)]}"
            else:
                # Use professional default response
                responses[field] = default_response
